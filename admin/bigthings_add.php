                ...
                <div class="col-xs-12">
                    <form id="add_article_form">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="event_date" placeholder="yyyy-mm-dd" autocomplete="off">
                        </div>
                        <div class="form-group">
                            <label for="content">事紀內容</label>
                            <textarea type="input" class="form-control" id="content" placeholder="請輸入內容"></textarea>
                        </div>
                        <div class="form-group">
    ...
    <script>
        $(document).on("ready", function() {
            ...
            $("#add_article_form").on("submit", function() {
                ...
                if ($("#event_date").val() == '' || $("#content").val() == '') {
                    alert("請填入或內文");

                    //清掉 loading icon
                    $("div.loading").html('');
                } else {
                    //使用 ajax 送出 帳密給 add_bigthings.php
                    $.ajax({
                        type: "POST",
                        url: "../php/add_bigthings.php", //因為此檔案是放在 admin 資料夾內，若要前往 php，就要回上一層 ../ 找到 php 才能進入 add_article.php
                        data: {
